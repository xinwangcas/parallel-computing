 ...
 */
HashTable<tsstring, tsstring>* hash_table;
...
void bench_init()
{
    hash_table = new HashTable<tsstring, tsstring>();

    // warm up the datastructure by filling it half full
    for (uint32_t w = 0; w < Config::CFG.elements; w+=2) {
	 tsstring skey; skey += (char)(32 + w%95);
	 tsstring sval; sval += (char)(32 + w%95);
...
//	cout << "To lookup element: " << skey << endl;
        hash_table->lookup(skey);
    }
    else if (act < Config::CFG.inspct){
//    	cout << "To insert element (" << skey << ", " << sval << ")" << endl;
        hash_table->insert(skey, sval);
//	hash_table->print();
    }
    else {
//    	cout << "To remove an element with key = " << skey << endl;
