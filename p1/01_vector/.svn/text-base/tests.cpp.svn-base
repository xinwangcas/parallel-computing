...
void run_custom_tests(config_t& cfg)
{
   ...
   //       that the lambda checks a member of each V
	...
	int value = rand_r(&cfg.seed);
	std::function<bool(pair<int, Nval>)> ff = [&value](pair<int, Nval> pp){
		if(pp.second.val == value)
		{
			cout << "match value: " << pp.second.val << endl;
			return true;
		}
		else 
		{ 
			cout << "dismatch value: " <<pp.second.val << endl;
			return false;
		};
	};
	if(nv.lookup_val(ff))
		cout<<"find value " << value << " succeed. "<< endl;
	else
		cout<<"find value " << value << " fail. "<< endl;
}
